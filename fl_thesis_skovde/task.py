...
def load_data(partition_id: int, num_partitions: int, alpha_partition: float):
    ...
    if fds is None:
        ...
        fds = FederatedDataset(
            dataset="uoft-cs/cifar10", # this can be find on Hugging Face 
            partitioners={"train": partitioner},
        )
    partition = fds.load_partition(partition_id)
    # Divide data on each node: 80% train, 20% test
    partition_train_test = partition.train_test_split(test_size=0.2, seed=42)
    # Here we define the transforms that will be applied to the data, check the dataset and change the transforms accordingly
